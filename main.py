...
def main():
    ...
    if prog_args.randGen:
        log_out_dir = prog_args.out_dir + '/' + time_mark + '_randGen_' + str(prog_args.seed)  + '_Normlize_'+ str(bool(prog_args.normalize))+'_concat_'+str(prog_args.concat) + '_log/'
    else:
        log_out_dir = prog_args.out_dir + '/' + time_mark + '_graphSize_' + str(prog_args.gs)  + '_Normlize_'+ str(bool(prog_args.normalize))+'_concat_'+str(prog_args.concat) + '_log/'
    if not os.path.exists(log_out_dir):
        os.makedirs(log_out_dir, exist_ok=True)
    # 定义 并创建 log 文件
    if prog_args.randGen:
        log_out_file = log_out_dir + time_mark + '_randGen_' + str(prog_args.seed)  + '_Normlize_'+ str(bool(prog_args.normalize)) +'_concat_'+str(prog_args.concat) + '_' + '_shuffle_' + str(prog_args.shuffle) + '.txt'

    else:
        log_out_file = log_out_dir + time_mark + '_graphSize_' + str(prog_args.gs)  + '_Normlize_'+ str(bool(prog_args.normalize)) +'_concat_'+str(prog_args.concat) + '_' + '_shuffle_' + str(prog_args.shuffle) + '.txt'
